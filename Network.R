...
library(ggpubr);require(cowplot);library(extrafont);library(dplyr);library(plotly) ;library(geomnet);
library(readxl)
...
ref = data.frame(
  ...
  category = rep("large",ncol(datExpr_low_new)))
test = data.frame(
  connectivity = as.numeric(Results_metab_cor$NC2$fundamentalNCs$Connectivity),
  category = rep("small",ncol(datExpr_high_new)))
...
Net_low_list_m = Net_low_list + 1 ; Net_high_list_m = Net_high_list +1
...
####################################################################################################
### now find the missing names in column x and swith with the corresponidng names in column y
###################################################
### aborted ###
############
names_inX = unique(Final_edgelist_low$allnames_label_new.x)
names_all = unique(c(Final_edgelist_low$allnames_label_new.x,Final_edgelist_low$allnames_label_new.y))
counter = 0
for (i in c(1:length(Final_edgelist_low$allnames_label_new.x))){
  Yname = Final_edgelist_low[i,5]
  Xname = Final_edgelist_low[i,1]
  if (length(which(names_inX == Yname)) == 0 & length(which(names_all == Yname)) > 0){
    counter = counter + 1
    Final_edgelist_low[i,5] = Xname
    Final_edgelist_low[i,1] = Yname
  }
}
length(unique(Final_edgelist_low$allnames_label_new.x))
length(unique(Final_edgelist_low$allnames_label_new.y))
length(intersect(Final_edgelist_low$allnames_label_new.x,Final_edgelist_low$allnames_label_new.y))
length(unique(c(Final_edgelist_low$allnames_label_new.x,Final_edgelist_low$allnames_label_new.y)))
